...
const ATTRIBUTE_MATCHERS: Array<[RegExp, TextInputLimitKey]> = [
  ...
  [/(product|item).*name|name.*(product|item)|sku|barcode|item.?name|product.?name/, 'PRODUCT_NAME'],
  [/(category).*name|name.*category|category.?name/, 'CATEGORY_NAME'],
  [/(attribute|addon|add.?on|modifier|option|subattribute).*name|name.*(attribute|addon|add.?on|modifier|option|subattribute)|attribute.?name|addon.?name|modifier.?name|subattribute.?name/, 'ATTRIBUTE_NAME'],
  [/(card.?type|cardtype).*name|name.*card.?type|card.?type.?name/, 'CARD_TYPE_NAME'],
  [/(payment).*name|name.*payment|payment.?name/, 'PAYMENT_NAME'],
  [/(discount|coupon|promo).*name|name.*(discount|coupon|promo)|discount.?name|promo.?name/, 'DISCOUNT_NAME'],
  [/(business|store|restaurant|location|establishment|brand).*name|name.*(business|store|restaurant|location|establishment|brand)|location.?name|brand.?name|store.?name/, 'BUSINESS_NAME'],
  [/^\s*name\s*$/, 'PERSON_NAME'],
  [/(first.?name|last.?name|full.?name|customer.?name|employee.?name|staff.?name|owner.?name|cardholder|card.?name|person.?name)/, 'PERSON_NAME'],
