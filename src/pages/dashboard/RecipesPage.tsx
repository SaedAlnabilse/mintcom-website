    ...
    <div className="space-y-8 pb-12">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          ...
          <p className="text-gray-500 dark:text-gray-400 font-medium mt-1">Design blueprints for menu items and sub-assemblies.</p>
        </div>
        <button
          onClick={() => {
            if (activeTab === 'final') {
      ...
      <div className="flex flex-col lg:flex-row gap-4 items-center justify-between">
        <div className="flex p-1.5 bg-white dark:bg-[#0A0A0A] border border-gray-100 dark:border-white/5 rounded-[1.5rem] shadow-sm">
          <button onClick={() => { setActiveTab('final'); setPage(1); }} className={`px-6 py-2.5 rounded-2xl text-xs font-black uppercase tracking-widest transition-all ${activeTab === 'final' ? 'bg-paymint-green text-black shadow-lg shadow-paymint-green/20' : 'text-gray-500 hover:text-gray-900 dark:hover:text-white'}`}>Finished Goods</button>
          <button onClick={() => { setActiveTab('sub'); setPage(1); }} className={`px-6 py-2.5 rounded-2xl text-xs font-black uppercase tracking-widest transition-all ${activeTab === 'sub' ? 'bg-paymint-green text-black shadow-lg shadow-paymint-green/20' : 'text-gray-500 hover:text-gray-900 dark:hover:text-white'}`}>Sub-Assemblies</button>
        </div>
        <div className="flex-1 w-full max-w-md relative group">
          ...
          <input type="text" value={searchQuery} onChange={(e) => { setSearchQuery(e.target.value); setPage(1); }} placeholder="Filter recipe blueprints..." className="w-full pl-12 pr-4 py-3.5 bg-white dark:bg-[#0A0A0A] border border-gray-100 dark:border-white/5 rounded-[1.25rem] text-gray-900 dark:text-white placeholder-gray-500 focus:ring-2 focus:ring-paymint-green/20 shadow-sm transition-all" />
        </div>
      </div>
      ...
      <AnimatePresence mode="wait">
        {isLoading ? (
          ...
          <div className="py-24 bg-white dark:bg-[#0A0A0A] rounded-[2.5rem] border border-gray-100 dark:border-white/5 text-center flex flex-col items-center">
          ...
          <div className="space-y-8">
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
              {paginatedItems.map((recipe: any) => (
                <motion.div layout key={recipe.id} className="group bg-white dark:bg-[#0A0A0A] p-8 rounded-[2.5rem] border border-gray-100 dark:border-white/5 shadow-sm hover:shadow-xl transition-all duration-300">
                  <div className="flex justify-between items-start mb-8">
                    ...
                    <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-all">
                      <button onClick={() => activeTab === 'final' ? openEditFinalRecipe(recipe) : openEditSubRecipe(recipe)} className="p-2 rounded-xl bg-gray-50 dark:bg-white/5 text-gray-400 hover:text-paymint-green"><Edit2 size={18} /></button>
                      <button onClick={() => handleDeleteRecipe(recipe.id, activeTab)} className="p-2 rounded-xl bg-gray-50 dark:bg-white/5 text-gray-400 hover:text-paymint-red"><Trash2 size={18} /></button>
                    </div>
                  </div>
                  <div className="space-y-3 mb-8 bg-gray-50 dark:bg-white/[0.02] p-5 rounded-[1.5rem] border border-gray-100 dark:border-white/5">
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-center gap-2 pt-4">
                <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page === 1} className="p-3 rounded-xl bg-white dark:bg-[#0A0A0A] border border-gray-100 dark:border-white/5 text-gray-500 hover:text-paymint-green disabled:opacity-30 transition-all"><ChevronLeft size={20} /></button>
                ...
                <button onClick={() => setPage(p => Math.min(totalPages, p + 1))} disabled={page === totalPages} className="p-3 rounded-xl bg-white dark:bg-[#0A0A0A] border border-gray-100 dark:border-white/5 text-gray-500 hover:text-paymint-green disabled:opacity-30 transition-all"><ChevronRight size={20} /></button>
              </div>
            )}
          </div>
        )}
      ...
      <AnimatePresence>
        {showSubRecipeModal && (
          <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="bg-white dark:bg-[#0A0A0A] rounded-[2.5rem] border border-gray-100 dark:border-white/5 w-full max-w-lg shadow-2xl flex flex-col overflow-visible">
              ...
              <div className="p-8 space-y-6">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input type="number" value={subRecipeForm.yield} onChange={(e) => setSubRecipeForm({ ...subRecipeForm, yield: parseFloat(e.target.value) || 0 })} className="w-full px-5 py-4 bg-gray-50 dark:bg-white/5 border-none rounded-2xl text-gray-900 dark:text-white font-black focus:ring-2 focus:ring-paymint-green/20 transition-all" />
                  </div>
                  <div>
                    <CustomSelect
                      label="Unit"
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-3">
                    {subRecipeForm.ingredients.map((ing, index) => (
                      <div key={index} className="flex gap-3 items-center p-3 bg-gray-50 dark:bg-white/[0.02] rounded-2xl border border-gray-100 dark:border-white/5">
      ...
      <AnimatePresence>
        {showFinalRecipeModal && (
          <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="bg-white dark:bg-[#0A0A0A] rounded-[2.5rem] border border-gray-100 dark:border-white/5 w-full max-w-lg max-h-[90vh] shadow-2xl flex flex-col overflow-visible">
              <div className="p-8 border-b border-gray-50 dark:border-white/5 flex items-center justify-between">
