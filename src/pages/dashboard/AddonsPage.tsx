    ...
    <div className="space-y-8 pb-12">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-gray-500 dark:text-gray-400 font-medium text-sm">Global item customizations, groups, and incremental pricing</p>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {[
          ...
          {
            label: 'Most Popular',
            value: stats.topGroup?.name || 'N/A',
            sub: `${stats.topGroup?.subAttributes?.length || 0} options`,
            icon: TrendingUp,
          ...
          <motion.div
            ...
            className="p-6 rounded-3xl bg-cream-50 dark:bg-[#0A0A0A] border border-cream-200 dark:border-white/5 shadow-md hover:shadow-lg hover:border-cream-300 dark:hover:border-white/10 transition-all"
          >
            <div className="flex items-center gap-4">
        ...
        <div className="space-y-4">
          {paginatedAttributes.map((attr) => (
            <motion.div
              layout
              key={attr.id}
              className="bg-cream-50 dark:bg-[#0A0A0A] rounded-[2rem] border border-cream-200 dark:border-white/5 overflow-hidden shadow-md hover:shadow-lg hover:border-cream-300 dark:hover:border-white/10 transition-all"
            >
              <div
                className="flex items-center justify-between p-6 cursor-pointer hover:bg-cream-100 dark:hover:bg-white/[0.02] transition-colors group"
                onClick={() => setExpandedId(expandedId === attr.id ? null : attr.id)}
              >
                <div className="flex items-center gap-5">
                  <div className={`w-12 h-12 rounded-xl flex items-center justify-center font-black border ${attr.isRequired ? 'bg-paymint-green text-black border-paymint-green' : 'bg-cream-100 dark:bg-white/5 text-gray-600 dark:text-gray-400 border-cream-200 dark:border-transparent'
                    }`}>
                ...
                <div className="flex items-center gap-4">
                  <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-all">
                    <button onClick={(e) => { e.stopPropagation(); openAttributeModal(attr); }} className="p-2 rounded-xl bg-cream-100 dark:bg-white/5 border border-cream-300 dark:border-transparent text-gray-600 dark:text-gray-400 hover:text-paymint-green hover:border-paymint-green/30">
                      <Edit2 size={16} />
                    </button>
                    <button onClick={(e) => { e.stopPropagation(); handleDeleteAttribute(attr.id); }} className="p-2 rounded-xl bg-cream-100 dark:bg-white/5 border border-cream-300 dark:border-transparent text-gray-600 dark:text-gray-400 hover:text-paymint-red hover:border-paymint-red/30">
                      <Trash2 size={16} />
                    </button>
                  </div>
              ...
              <AnimatePresence>
                {expandedId === attr.id && (
                  <motion.div
                    ...
                    className="border-t border-cream-200 dark:border-white/5 bg-cream-100/50 dark:bg-black/20 p-8"
                  >
                    <div className="flex items-center justify-between mb-8">
                      ...
                      <button
                        ...
                        className="px-4 py-2 bg-cream-50 dark:bg-white/5 border border-cream-300 dark:border-white/10 text-gray-900 dark:text-white text-xs font-black uppercase tracking-widest rounded-xl hover:bg-paymint-green hover:text-black hover:border-paymint-green transition-all flex items-center gap-2"
                      >
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                      {attr.subAttributes?.map((sub) => (
                        <div key={sub.id} className="flex items-center justify-between p-4 bg-cream-50 dark:bg-[#0A0A0A] rounded-2xl border border-cream-200 dark:border-white/5 group/sub hover:border-cream-300 dark:hover:border-white/10 transition-all">
                          <div>
        ...
        {showAttributeModal && (
          <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="bg-cream-50 dark:bg-[#0A0A0A] rounded-[2.5rem] border border-cream-300 dark:border-white/5 w-full max-w-md shadow-2xl overflow-hidden">
              <div className="p-8 border-b border-cream-50 dark:border-white/5 flex items-center justify-between">
              ...
              <div className="p-8 space-y-8">
                <div>
                  ...
                  <input
                    ...
                    className={`w-full px-5 py-4 bg-cream-100 dark:bg-[#1a1a1a] border ${errors.groupName ? 'border-red-500 ring-2 ring-red-500/20' : 'border-cream-300 dark:border-white/10'} rounded-2xl text-gray-900 dark:text-white font-black focus:outline-none focus:ring-2 focus:ring-paymint-green/20 transition-all`}
                ...
                <div className="grid grid-cols-2 gap-4">
                  <button
                    ...
                    className={`p-5 rounded-3xl border-2 transition-all flex flex-col items-center gap-3 ${attributeForm.inputType === 'SINGLE_SELECT' ? 'border-paymint-green bg-paymint-green/5 text-paymint-green' : 'border-cream-200 dark:border-white/5 text-gray-400'}`}
                  >
                  ...
                  <button
                    ...
                    className={`p-5 rounded-3xl border-2 transition-all flex flex-col items-center gap-3 ${attributeForm.inputType === 'MULTI_SELECT' ? 'border-paymint-green bg-paymint-green/5 text-paymint-green' : 'border-cream-200 dark:border-white/5 text-gray-400'}`}
                  >
                    <Grid size={24} />
                ...
                <div className="flex items-center justify-between p-5 bg-cream-100 dark:bg-white/5 rounded-[1.5rem] border border-cream-200 dark:border-white/5">
                  <div>
              ...
              <div className="p-8 space-y-8">
                <div>
                  ...
                  <input
                    ...
                    className={`w-full px-5 py-4 bg-cream-100 dark:bg-[#1a1a1a] border ${errors.optionName ? 'border-red-500 ring-2 ring-red-500/20' : 'border-cream-300 dark:border-white/10'} rounded-2xl text-gray-900 dark:text-white font-black focus:outline-none focus:ring-2 focus:ring-paymint-green/20 transition-all`}
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-5 py-4 bg-cream-100 dark:bg-[#1a1a1a] border border-cream-300 dark:border-white/10 rounded-2xl text-gray-900 dark:text-white font-black focus:ring-2 focus:ring-paymint-green/20 transition-all"
                  />
                </div>
