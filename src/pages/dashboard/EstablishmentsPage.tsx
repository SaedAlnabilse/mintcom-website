...
export function EstablishmentsPage() {
  ...
  const getStatusColor = (status: string) => {
    switch (status?.toLowerCase()) {
      case 'active':
        return 'text-mintcom-green bg-mintcom-green/10 border-mintcom-green/20';
      case 'trial':
      case 'trialing':
        return 'text-yellow-500 bg-yellow-500/10 border-yellow-500/20';
      case 'past_due':
        return 'text-mintcom-red bg-mintcom-red/10 border-mintcom-red/20';
      case 'canceled':
        ...
        <div className="p-8">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {paginatedEstablishments.map((est) => (
              ...
              >
                {/* Background Effects */}
                <div className={`absolute top-0 ${t('common.locale') === 'ar' ? 'left-0' : 'right-0'} w-32 h-32 rounded-full blur-3xl opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none ${currentEstablishment?.id === est.id ? 'bg-mintcom-green/10 opacity-100' : 'bg-mintcom-green/5'
                  }`} />
